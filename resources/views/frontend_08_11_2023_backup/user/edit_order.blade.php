            ...
            <div class="container-fluid">
                <div class="section-title-3 text-center mb-40">
                    ...
                    <div class="section-title-3 text-center mb-15 custmizeMargin" role="tablist">
                        <div class="menu-custbtn-area text-center mt-1" style="display: inline-block; color:white;">
                            <a class="menu-custbtn" href="#">Edit Order</a>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="contact-input-style mb-30">
                                ...
                                <select class="form-select single-select-field " data-placeholder="Choose one thing" name="deliver_from" onchange="getState(this.value,'devliver_fromOrder')">
                                    @foreach($country as $r)
                                        <option <?php if($data->origin_country==$r->id){ echo "selected"; } ?> value="{{$r->id}}">{{$r->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="contact-input-style mb-30">
                            ...
                            <div class="contact-input-style mb-30">
                                ...
                                <select class="form-select single-select-field " data-placeholder="Choose one thing" name="deliver_toOrderCity" id="deliver_toOrderCity">
                                    @foreach($state_to as $r)
                                        <option <?php if($data['destination']['country_id']==$r->id){ echo "selected"; } ?> value="{{$r->id}}">{{$r->city_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="contact-input-style mb-30">
                                <label>How long are you willing to wait?</label>
                                <input name="during_date"  type="date"  min="<?php echo date('Y-m-d'); ?>"  value="<?php echo date('Y-m-d',strtotime($data->deliverd_date)) ?>" >
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="hidden" name="id" value="{{$data->id}}">
                                <input name="product_name" required="" type="text" class="form-control textFx" id="product_name" placeholder="Enter Product Name" value="{{$product_data->product_name}}">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="mt-2">
                                    @foreach($img as $i)
                                        ...
                                        <a href="#">
                                        <img src="{{ URL::to('/') }}/public/product_img/{{ $i }}">
                                            </a>
                                    </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input name="product_qty" required="" type="number" class="form-control textFx" id="product_qty" placeholder="Enter Product Quantity" value="{{$data->quantity}}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select textFx" data-placeholder="Choose one thing" name="deliver_toOrderCity" id="deliver_toOrderCity">
                                    @foreach($state_to as $r)
                                        <option <?php if($data->destination_city==$r->id){ echo "selected"; } ?> value="{{$r->id}}">{{$r->city_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input name="during_date" type="date" min="<?php echo date('Y-m-d'); ?>" class="form-control textFx" value="<?php echo date('Y-m-d',strtotime($data->deliverd_date)) ?>">
                            </div>
                            <div class="mb-3">
                                <label for="product_details" class="form-label labelX">Product Details</label>
                                <textarea name="product_details" required="" class="form-control textFx heightArea" id="product_details">{{$product_data->product_details}}</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="mb-3">
                                <div class="menu-custbtn-area text-center mt-1" >
                                <button  type="submit" class="menu-custbtn" style="border:none">Update Order</button>
                            </div>
                            </div>
                </form>
