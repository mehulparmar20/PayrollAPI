...
class CompanyEmployeeController extends Controller
{
    ...
    public function delete_employee(Request $request) //done
    {
        ...
        $userData = Company_Employee::raw()->updateOne(
            ['company_id' => $companyID, '_id' => $id],
    ...
    public function paginate_employee(Request $request)
    {
        for ($i = 0; $i < sizeof($paginate[0][0][0]); $i++) {
            ...
            $show1 = Company_user::raw()->aggregate([
                ...
                ['$project' => [
                    "company_user.userId" => 1, "company_user._id" => 1, "company_user.counter" => 1, "company_user.custName" => 1, "company_user.custLocation" => 1, "company_user.custLocationCity" => 1, "company_user.custLocationState" => 1, "company_user.custZip" => 1, "company_user.primaryContact" => 1,
    ...
    public function employee_history(Request $request)
    {
        ...
        list($id, $user, $admin_name, $companyname) = explode('|', $decryptedInput);
        $companyId = intval($id);
        $existingemployeeData = Company_Employee::where('company_id',$companyId)->select('first_name', 'last_name','email','joining_date','delete_status')->get();
        //dd($existingemployeeData);
        if ($existingemployeeData != '') {
            return response()->json([
                'success' => $existingemployeeData,
